...
            <td>{{ list.boardwriter }}</td>
            <!--
                list에서 해당 글 제목을 클릭하게 되면 상세보기로 넘어가는 코드를 구현해보겠습니다.
                <router-link>를 통해서 /board/detaul이라는 name을 보내주고, parameter로 boardno라는 이름으로
                list.boardno의 값을 보내줍니다.
                name이라는 것은 index.js에서 path를 어떤 이름으로 받아줄거냐의 속성입니다.

                저는 goDetail이라는 이름으로 지정을 하였습니다. 그럼 이는 router를 지정해주는 index.js에서
                name 속성을 goDetail로 지정해주면 됩니다.
            -->
            <td><router-link :to="{name: 'goDetail', params: {boardno: list.boardno}}" >{{ list.boardtitle }}</router-link></td>
            <!-- <td>{{ list.boardtitle }}</td> -->
            <textarea v-model="list.boardcontent"></textarea>
